...
ShapercurveComponent::ShapercurveComponent(PlasmaAudioProcessor& p, int stage)
  : audioProcessor(p)
{
...
void
ShapercurveComponent::parameterValueChanged(int parameterIndex, float newValue)
{
  parametersChanged.set(true);
}

void
ShapercurveComponent::timerCallback()
{
...
void
ShapercurveComponent::paint(juce::Graphics& g)
{
  ...
  for (int i = 0; i < w; ++i) {
    ...
    // Girth
    samples[i] = samples[i] * ((random.nextFloat() * abs(girth)) + 1);
...
void
ShapercurveComponent::setColor(Colour c)
{
  color = c;
}
